...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        """
        if insect.is_ant: #if insect to be removed is ant
            # Phase 4: Special Handling for BodyguardAnt and QueenAnt
            if isinstance(insect, QueenAnt) and insect.real == True:
                return
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        count = 0
        if self.min_range:
            for x in range(0, self.min_range):
        ...
        while current.bees == []:
            ...
            if current == None:
                return None
            count += 1
        if self.max_range and count > self.max_range:
            return None
        if current == hive:
            return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            myPlace.remove_insect(self)
            myBees = list(myPlace.bees)
            for b in myBees:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.real == False:
            ...
            while behind:
                if behind.ant:
                    if behind.ant not in self.doubled:
                        behind.ant.damage *=2
                        self.doubled.append(behind.ant)
                    if isinstance(behind.ant, BodyguardAnt) and behind.ant.ant != None and behind.ant.ant not in self.doubled:
                        behind.ant.ant.damage *=2
                        self.doubled.append(behind.ant.ant)
                behind = behind.exit
    ...
    def reduce_armor(self, amount):
        ...
        "*** REPLACE THIS LINE ***"
        self.armor -= amount
        if self.real == True and self.armor <= 0:
            bees_win()
        elif self.armor <= 0:
            self.place.remove_insect(self)
